...
public class TestCasesP1
{
   ...
   {
      assertEquals(2.2, SimpleIf.max(2.2, 2.2), DELTA);
   }
   ...
   {
      assertEquals(1, SimpleLoop.sum(0, 1));
   }
   ...
   public void testSimpleArray3()
   {
      assertArrayEquals(
              new int[] {64, 144, 0, 1},
              SimpleArray.squareAll(new int[] {8, 12, 0, 1}));
   }
   ...
   {
      List<Integer> input =
              new LinkedList<Integer>(Arrays.asList(new Integer[] {1, 4, 9}));
      List<Integer> expected =
              new ArrayList<Integer>(Arrays.asList(new Integer[] {1, 16, 81}));
   ...
   @Test
   public void testBetterLoop3() { assertFalse(BetterLoop.contains(new int[] {7, 5, 2, 4}, 1)); }
   ...
   {
      List<String> expected = Arrays.asList("Paul");
      Map<String, List<Course>> courseListsByStudent = new HashMap<>();
